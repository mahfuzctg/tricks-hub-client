...
const VoteSection = ({postId, userId, votes, voters }: { postId : string, userId: string, votes: number, voters: { userId: string, voteType : string }[]}) => {
  ...
  <button onClick={() => handleVote('upvote')}><PiArrowFatUpLight className={`text-gray-800 dark:text-gray-400  hover:text-gray-600 cursor-pointer hover:scale-110 transition-transform text-xl  ${givenVote?.voteType === 'upvote' && 'text-gray-600'} `} /></button>}
  ...
  <button onClick={() => handleVote('downvote')}>
    ...
    <PiArrowFatDownLight className={`text-gray-800 dark:text-gray-400 hover:text-gray-600 cursor-pointer hover:scale-110 transition-transform text-xl  ${givenVote?.voteType === 'downvote' && 'text-gray-600'}`} />}
