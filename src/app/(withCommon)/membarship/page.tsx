    ...
    <div className="flex flex-col items-center justify-center p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6 w-full">
        {membershipPackages.map((pack, index) => (
          <div
            key={index}
            className="bg-white dark:bg-gray-800 shadow-lg rounded-2xl p-6 flex flex-col justify-between hover:shadow-2xl transition-shadow duration-300 transform hover:scale-105"
            style={{
              backgroundColor: pack.bgColor,
            }}
          >
            <div className="text-center mb-4">
              <h2 className={`text-2xl font-semibold text-${pack.color}`}>{pack.name}</h2>
            ...
            <button
              ...
              className="w-full py-2 px-4 text-white rounded-md mt-auto hover:bg-opacity-90 transition-colors"
              style={{ backgroundColor: '#4B5563' }} // Dark gray button for all
            >
              Subscribe Now
            </button>
          </div>
